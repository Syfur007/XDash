...
worker registry retired (§3.7). `colab.py` (Phase 4) subclasses `machine.py`'s
`MachineRunner`: a Colab VM is an ordinary SSH host from the moment
dispatch() provisions it, so only provisioning/teardown and Colab's own
eligibility checks live there. No facade holds its own state or duplicates
...
from .colab import ColabRunner, list_colab_runners

__all__ = [
    "CapacitySnapshot", "LaunchSpec", "Runner", "RunnerCapabilities", "RunnerCapabilityError", "RunUnit",
    "MachineRunner", "KaggleRunner", "ColabRunner", "list_runners", "get_runner",
