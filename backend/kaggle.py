...
# Five extra ways to hear about a finished/errored worker, on top of the
# generic incoming-webhook above and the client-side browser Notification in
# kaggle.js — all server-side, so they fire from the background poller (see
# _tick below) even when nobody has the dashboard tab open. Settings live in
# their own gitignored JSON file (never dashboard_config.yaml — these are
# runtime-editable from the UI, not deploy-time config) and secret fields
# (bot tokens, webhook URLs, SMTP passwords) are never echoed back to a
...
def _load_notifications() -> Dict[str, Any]:
    path = settings.kaggle_notifications_file
...
def update_notification_settings(channel: str, patch: Dict[str, Any]) -> Dict[str, Any]:
    if channel not in NOTIFICATION_CHANNELS:
        raise KaggleOpsError(f"Unknown notification channel: {channel}")
    with _notif_lock:
        data = _load_notifications()
        cfg = data[channel]
        secret_fields = _NOTIF_SECRET_FIELDS.get(channel, set())
...
def _send_telegram(cfg: Dict[str, Any], text: str) -> None:
    ...
    if not token or not chat_id:
        raise KaggleOpsError("Telegram is missing a bot token or chat ID.")
    req = urllib.request.Request(
...
def _send_discord(cfg: Dict[str, Any], text: str) -> None:
    ...
    if not url:
        raise KaggleOpsError("Discord is missing a webhook URL.")
    # Discord's webhook API reads the message body from "content", not
    # "text" — the generic _send_webhook() above uses "text", which is why
    # it only actually renders a message on Slack, not Discord, despite its
    # docstring calling itself "Slack/Discord-compatible".
    req = urllib.request.Request(
...
def _send_slack(cfg: Dict[str, Any], text: str) -> None:
    url = (cfg.get("webhook_url") or "").strip()
    if not url:
        raise KaggleOpsError("Slack is missing a webhook URL.")
    req = urllib.request.Request(
...
def _send_email(cfg: Dict[str, Any], text: str) -> None:
    ...
    if not host or not to_addr:
        raise KaggleOpsError("Email is missing an SMTP host or a recipient address.")
    user = (cfg.get("smtp_user") or "").strip()
    password = cfg.get("smtp_password") or ""
    from_addr = (cfg.get("from_addr") or "").strip() or user or to_addr
    port = int(cfg.get("smtp_port") or 587)
    msg = MIMEText(text)
    msg["Subject"] = "Kaggle worker update"
    msg["From"] = from_addr
    msg["To"] = to_addr
...
def _send_ntfy(cfg: Dict[str, Any], text: str) -> None:
    ...
    if not topic:
        raise KaggleOpsError("ntfy is missing a topic.")
...
def test_notification(channel: str) -> Dict[str, Any]:
    ...
    try:
        ...
        raise KaggleOpsError(f"{channel} test failed: {e}")
    return {"ok": True}


def _send_notifications(text: str) -> None:
    """Best-effort fan-out to every enabled channel, plus the legacy
    dashboard_config.yaml webhook. Never raises — same reasoning as
    _send_webhook above: a notification failure must never take down a poll
    tick."""
    data = _load_notifications()
...
def _tick() -> None:
    ...
    for account in data["accounts"]:
        for w in account.get("workers", []):
            ...
            _send_notifications(f"Kaggle worker '{worker_id}' ({account['name']}) is now {new_status}.")
